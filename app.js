...
let state = {
  ...
  // Vehicle gate — SISTEM VERIFIKASI 2 LANGKAH:
  // Langkah 1 (Ultrasonik, di luar): objek mendekat -> gerbang buka sendiri.
  // Langkah 2 (IR, di dalam garasi): kendaraan terdeteksi masuk -> gerbang
  // baru menutup (ada timer fallback kalau IR tidak kunjung mendeteksi).
  // Kalau ultrasonicEnabled = false, gerbang murni manual (tombol).
  distance: 45.0,
  vehicle: false,
  gate: "CLOSED",
  ultrasonicEnabled: false,
  irDetected: false,   // sensor IR di dalam garasi (langkah 2 verifikasi)
...
window.toggleGarageLamp = function (enabled) {
  if (publishCommand("garageLamp", enabled)) {
    ...
    toast("Lampu Garasi & Rumah", enabled ? "Lampu garasi & lampu rumah (DC) dinyalakan." : "Lampu garasi & lampu rumah (DC) dimatikan.");
  }
};
...
function updateUI() {
  ...
  // IR sensor (langkah 2 verifikasi, di dalam garasi)
  if ($("irBadge")) {
    $("irBadge").textContent = state.irDetected ? "TERDETEKSI" : "CLEAR";
    $("irBadge").className = `badge ${state.irDetected ? "success" : "neutral"} text-[10px]`;
  }
  if ($("irHint")) {
    $("irHint").textContent = !state.ultrasonicEnabled
      ? "Nonaktif (mode manual)"
      : (state.irDetected ? "Kendaraan terdeteksi — gerbang akan menutup sebentar lagi." : "Menunggu kendaraan masuk");
  }
  if ($("irPad")) $("irPad").classList.toggle("active", !!state.irDetected);
