...
window.sendGate = function (value) {
  if (publishCommand("gate", value)) {
    addActivity(`Perintah gerbang utama: ${value.toUpperCase()}`, "command");
    toast("Gate Command", `Gerbang utama ${value === "open" ? "dibuka" : "ditutup"}.`);
  }
};
...
window.toggleLamp = function (number, value) {
  if (publishCommand(`lamp${number}`, value)) {
    addActivity(`Lampu Ruang ${number}: ${value ? "ON" : "OFF"}`, "command");
  }
};
...
function updateSecurityUI() {
  ...
  $("securityTouchState").textContent = state.touchEnabled ? "ARMED" : "OFF";
  $("securityTouchState").className = `badge ${state.touchEnabled ? "success" : "neutral"}`;
}
